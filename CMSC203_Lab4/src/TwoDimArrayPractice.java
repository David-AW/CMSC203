...
public class TwoDimArrayPractice extends JFrame
{
	...
	public void printArray( )
	{
		...
		for (int y = 0; y < intArray.length; y++) {
			...
			for (int x = 0; x < intArray[y].length; x++) {
				System.out.print(intArray[y][x] + (x < intArray[y].length-1 ? " " : "\n"));
				animate(y, x);
			}
		}
	...
	public void setValues( int value, int row )
	{
		...
		for (int x = 0; x < intArray[row].length; x++) {
	...
	public int findMinimum( int column )
	{
		...
		for (int y = index; y < intArray.length; y++) {
			if (intArray[y][column] < min) {
				min = intArray[y][column];
				index = y;
			}
			animate(y, column, min);
		}
